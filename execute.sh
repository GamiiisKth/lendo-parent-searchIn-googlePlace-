...
##############################################################################
#
# The script works by executing a command in each repository according to the
# order specified in the buildOrder array specified at the top of the script
#
...
case "$1" in 
  ...
  "build")
    start=`date +%s`
    execute mvn clean install -T 1C
    end=`date +%s`
    buildtime=$((end-start))
    echo "Build time $(($buildtime / 60)) minutes $(($buildtime % 60)) seconds"
    ;;
  "push")
    execute git push
    ;;    
